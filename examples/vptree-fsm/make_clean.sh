...
~/pyCuAsm/pyCuAsm.py vptree.sass -r 4 -t 256 $@
#sed -i -e 's/-0.5/0.5.NEG/g' out.sass

if [ -f out.sass ]
then
	maxas.pl -i out.sass kernel.sm_52.cubin
	~/pyCuAsm/pyCuAsm.py -e -k _Z13search_kernel10__GPU_treeP5PointP11__GPU_pointS1_ kernel.sm_52.cubin -o vptree.out.sass
fi
