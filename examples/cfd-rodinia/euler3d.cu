...
__global__ void cuda_initialize_variables(int nelr, float* variables, float ff_variable[NVAR])
{
...
inline void compute_flux_contribution_h(float& density, float3& momentum, float& density_energy, float& pressure, float3& velocity, float3& fc_momentum_x, float3& fc_momentum_y, float3& fc_momentum_z, float3& fc_density_energy)
{
...
__attribute__((always_inline)) __device__ __inline__ void compute_flux_contribution(float& density, float3& momentum, float& density_energy, float& pressure, float3& velocity, float3& fc_momentum_x, float3& fc_momentum_y, float3& fc_momentum_z, float3& fc_density_energy)
{
...
__attribute__((always_inline)) __device__ __inline__ void compute_velocity(float& density, float3& momentum, float3& velocity)
{
	extern __shared__ float s[];
...
__attribute__((always_inline)) __device__ __inline__ float compute_speed_sqd(float3& velocity)
{
	extern __shared__ float s[];
	return velocity.x*velocity.x + velocity.y*velocity.y + velocity.z*velocity.z;
}

__attribute__((always_inline)) __device__ __inline__ float compute_pressure(float& density, float& density_energy, float& speed_sqd)
{
	extern __shared__ float s[];
	return (float(GAMMA)-float(1.0f))*(density_energy - float(0.5f)*density*speed_sqd);
}

__attribute__((always_inline)) __device__ __inline__ float compute_speed_of_sound(float& density, float& pressure)
{
	extern __shared__ float s[];
...
__global__ void cuda_compute_flux(int nelr, int* elements_surrounding_elements, float* normals, float* variables, float* fluxes,
	float ff_variable[NVAR], float3 ff_flux_contribution_momentum_x, float3 ff_flux_contribution_momentum_y, float3 ff_flux_contribution_momentum_z, 
	float3 ff_flux_contribution_density_energy)
{
...
void compute_flux(int nelr, int* elements_surrounding_elements, float* normals, float* variables, float* fluxes)
{
	...
	cuda_compute_flux<<<Dg,Db,12288>>>(nelr, elements_surrounding_elements, normals, variables, fluxes, ff_variable, ff_flux_contribution_momentum_x, ff_flux_contribution_momentum_y, ff_flux_contribution_momentum_z, ff_flux_contribution_density_energy);
//	cuda_compute_flux<<<Dg,Db,9216>>>(nelr, elements_surrounding_elements, normals, variables, fluxes, ff_variable, ff_flux_contribution_momentum_x, ff_flux_contribution_momentum_y, ff_flux_contribution_momentum_z, ff_flux_contribution_density_energy);
...
int main(int argc, char** argv)
{
  printf("WG size of kernel:initialize = %d, WG size of kernel:compute_step_factor = %d, WG size of kernel:compute_flux = %d, WG size of kernel:time_step = %d\n", BLOCK_SIZE_1, BLOCK_SIZE_2, BLOCK_SIZE_3, BLOCK_SIZE_4);
	...
	// set far field conditions and load them into constant memory on the gpu
	{
		...
		compute_flux_contribution_h(h_ff_variable[VAR_DENSITY], h_ff_momentum, h_ff_variable[VAR_DENSITY_ENERGY], ff_pressure, ff_velocity, h_ff_flux_contribution_momentum_x, h_ff_flux_contribution_momentum_y, h_ff_flux_contribution_momentum_z, h_ff_flux_contribution_density_energy);

		cudaMalloc(&ff_variable, NVAR * sizeof(float));
		// copy far field conditions to the gpu
		checkCudaErrors( cudaMemcpy(ff_variable,          h_ff_variable,          NVAR*sizeof(float), cudaMemcpyHostToDevice ));	
		
		ff_flux_contribution_momentum_x = h_ff_flux_contribution_momentum_x;
		...
		ff_flux_contribution_density_energy = h_ff_flux_contribution_density_energy;	
	}
	int nel;
	int nelr;
