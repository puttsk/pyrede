                             ...
                             unsigned int foundDigest[4])
{
    ...
    FindKeyWithDigest_Kernel<<<nblocks, nthreads, nthreads * sizeof(float) * 6>>>(searchDigest[0],
                                                    searchDigest[1],
                                                    searchDigest[2],
                                                    searchDigest[3],
                                                    keyspace,
...
RunBenchmark(ResultDatabase &resultDB, OptionParser &op)
{
    ...
    //
    const int sizes_byteLength[]  = { 7,  5,  6,  5,  5};
    const int sizes_valsPerByte[] = { 10, 35, 25, 70, 72};
