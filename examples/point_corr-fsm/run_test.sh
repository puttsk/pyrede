...
do
    make clean;
    make ${MAKE_ARGS[*]} DIM=${INPUT_DIM[$j]} RADIUS=${INPUT_RAD[$j]}
    f=$(basename $file)
    ...
    do
        ...
        ./pc $file $npoints 2>&1 | ${SCRIPT_DIR}collect.py ${TMP_DIR}${APP}_${TYPE}_${f}_unsorted.stats 2>&1 >> ${TMP_DIR}${APP}_${TYPE}_${f}_unsorted.out 
        ./pc -s $file $npoints 2>&1 | ${SCRIPT_DIR}collect.py ${TMP_DIR}${APP}_${TYPE}_${f}_sorted.stats 2>&1 >> ${TMP_DIR}${APP}_${TYPE}_${f}_sorted.out
    done
